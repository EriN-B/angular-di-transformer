...
export function checkArgs(args: string[] | undefined, sourceFile: SourceFile) {
  if (args) {
    return args.some((substring) =>
      sourceFile.getBaseName().includes(substring)
    );
  } else {
    return true;
  }
  ...
  const args = parse<ICopyFilesArguments>(
    {
      constr: {
        type: Boolean,
        alias: 'c',
        description: 'Do refactoring of DP, even if constructor contains code',
        optional: true,
        defaultValue: false
      },
      scheme: {
        type: String,
        alias: 's',
        description: `(optional) File Type to transform. \n Examples: ${blue(
          '-s component service'
        )} \n`,
        multiple: true,
        optional: true
