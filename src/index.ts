...
import {
  getAngularVersion,
  isAngularWorkspace
} from './utils/check-angular-workspace';
...
function main() {
  ...
  if (!isWorktreeClean()) {
    console.error(
      'Your Git worktree is not clean. Please commit or stash your changes before running this script.'
    );
    process.exit(1);
  }
  ...
  if (!isAngularWorkspace() && getAngularVersion() >= 14) {
    console.error(
      'Please run this command inside of a Angular Workspace with a version of Angular/core of >= 14'
    );
    process.exit(1);
  }
  ...
  project.getSourceFiles().forEach((sourceFile) => {
    try {
      ...
      console.error(
        `Error processing file ${sourceFile.getFilePath()}: ${error}`
      );
    }
  });
    ...
    sourceFile.getClasses().forEach((classDeclaration) => {
      classDeclaration.getConstructors().forEach((constructorDeclaration) => {
        ...
        if (shouldRefactorConstructor(constructorBody, args)) {
          ...
          console.log(
            yellow(
              `Constructor of ${sourceFile.getBaseName()} has descendant Statements. Skipping refactoring of constructor. You can disable this behaviour with the -c option`
            )
          );
        }
      });
  ...
  function refactorConstructor(
    ...
    constructorDeclaration.getParameters().forEach((parameterDeclaration) => {
      ...
      insertPropertyAndRemoveParameter(
        classDeclaration,
        name,
        type,
        parameterDeclaration
  ...
  function shouldRefactorConstructor(
    constructorBody: any,
    args: { scheme: string[] | undefined; constr: boolean | undefined }
  ) {
    return (
      (constructorBody &&
        constructorBody.getDescendantStatements().length === 0) ||
      args.constr
    );
  }

  function insertPropertyAndRemoveParameter(
    classDeclaration: ClassDeclaration,
    name: string,
  ...
  function ensureInjectImport(sourceFile: SourceFile) {
    ...
    const foundInjectImport = angularCoreImport
      ?.getNamedImports()
  ...
  function removeConstructorIfEmpty(
    constructorDeclaration: ConstructorDeclaration,
    constructorBody: any
  ) {
    if (
      constructorDeclaration.getParameters().length === 0 &&
      constructorBody &&
      constructorBody.getDescendantStatements().length === 0
    ) {
